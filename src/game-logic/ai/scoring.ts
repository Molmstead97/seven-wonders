...
function calculatePlayScore(
  ...
  if (card.production) {
    ...
    if (card.production.choice) {
      ...
      score += baseResourceScore + scoreChoiceProduction(
  ...
  if (card.science) {
    ...
    if (personality.scienceFocus > 0.6) { // Only heavily invest if science-focused
      if (competingScience < 2) { // Low competition
    ...
    } else { // For non-science focused AI
      if (existingScienceTypes > 0) { // Already have some science
...
function calculateWonderScore(
  ...
  const builtStages = player.wonder.wonderStages.filter(s => s.isBuilt).length;
...
function calculateDiscardScore(
  ...
  Object.entries(wonderNeeds).forEach(([resource, need]) => {
    if (need.priority > 5) { // High priority needs
      score -= need.priority;
    }
  });
  ...
  ["Wood", "Stone", "Clay", "Ore", "Glass", "Papyrus", "Textile"].forEach(
    (resource) => {
      ...
      tradingOptions.set(resource, {
        leftCost: calculateTradeCost(player, player.leftPlayer!, resource as keyof Resource),
        rightCost: calculateTradeCost(player, player.rightPlayer!, resource as keyof Resource),
...
function calculateNeighborProduction(player: Player, resource: keyof Resource): number {
  let production = 0;

  // Only count fixed production from cards (not wonder stages or choice production)
  player.playerBoard.forEach(card => {
    if (card.production && !card.production.choice) {
...
function calculateTradeCost(player: Player, neighbor: Player, resource: keyof Resource): number {
  // Base trading cost is 2
  let cost = 2;
  
  // Check for trading post effects (yellow cards that reduce trading costs)
  player.playerBoard.forEach(card => {
    if (card.specialEffect?.type === "tradeDiscount" && 
        card.specialEffect.neighbor === (neighbor === player.leftPlayer ? "left" : "right")) {
      cost = 1;
    }
  });
...
function determineAvailability(leftProduction: number, rightProduction: number): "high" | "medium" | "low" {
...
function scoreTradingOption(resource: string, tradingOptions: Map<string, any>): number {
    const option = tradingOptions.get(resource);
    if (!option) return 0;
  
    // Higher score for cheaper trading costs and better availability
    const costScore = Math.max(0, 5 - Math.min(option.leftCost, option.rightCost));
    ...
    return costScore + availabilityScore[option.availability as keyof typeof availabilityScore];
  }
...
function analyzeCompetingScience(player: Player, gameState: GameState): number {
  ...
  gameState.players.forEach(otherPlayer => {
    if (otherPlayer !== player) {
      const scienceCount = Object.values(otherPlayer.science).reduce((sum, count) => sum + count, 0);
      if (scienceCount > 1) { // Consider players with 2+ science symbols as competition
        competingPlayers++;
      }
    }
  });
