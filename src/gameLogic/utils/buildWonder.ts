...
export function buildWonder(
  ...
  if (checkResources(player, nextStage)) {
    ...
    if (nextStage.specialEffect) {
      applySpecialEffect(updatedPlayer, nextStage.specialEffect, gameState, card);
    }
...
function applySpecialEffect(player: Player, effect: SpecialEffect, gameState: GameState, card: Card) {
  switch (effect.type) {
